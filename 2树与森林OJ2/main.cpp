...
public:
    ...
    void setLChild (TreeNode* Node) { _leftChild = Node;    if (Node != nullptr)        Node->setParenet (this); }
    void setRChild (TreeNode* Node) { _rightChild = Node;    if (Node != nullptr)        Node->setParenet (this); }
...
template <typename T>
//应该也是递归
void TreeNode<T>::ConvertToBTree () {
...
~Tree () { if (_root != nullptr) delete _root; _root = nullptr; }
...
int main () {
    ...
    if (K == 0) {
        ...
        for (i = 0; i < N; i++) {
            cin >> A >> l >> r;
            nodes[A].setIndex (A);
            if (l == -1) nodes[A].setLChild (nullptr);
            else  nodes[A].setLChild (&nodes[l]);

            if (r == -1) nodes[A].setRChild (nullptr);
            else nodes[A].setRChild (&nodes[r]);
        }
    ...
    for (i = 0; i < Q; i++) {
        ...
        if (op == 1) {
            ...
            cin >> pos >> father >> node;
            if (pos == 0)  tree->addRightSubTreeByIndex (&nodes[node], father);
            else tree->addLeftSubTreeByIndex (&nodes[node], father);
        } else {
            if (K == 0)tree->TreePreorderTraversal ();
            else tree->BiPreorderTraversal ();
        }
